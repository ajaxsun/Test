...
import static com.example.myapplication.pickerview.helper.Utils.TYPE_BIRTH;
import static com.example.myapplication.pickerview.helper.Utils.TYPE_NORMAL;
import static com.example.myapplication.pickerview.helper.Utils.getCurrentDay;
import static com.example.myapplication.pickerview.helper.Utils.getCurrentMonth;
import static com.example.myapplication.pickerview.helper.Utils.getCurrentYear;
import static com.example.myapplication.pickerview.helper.Utils.getYearList;
import static com.example.myapplication.pickerview.helper.Utils.isRunYear;
import static com.example.myapplication.pickerview.helper.Utils.setBackgroundAlpha;
...
public class PopDateHelper extends PopupWindow {
    ...
    private void initData() {
        ...
        list_month = Utils.getMonthList();
        list_day =  Utils.getDaysList();
    }
            ...
            public void onItemSelected(int index) {
                ...
                if (!TextUtils.isEmpty(month) && month.equals("02")) {
                    if (isRunYear(year) && list_day.size() != 29) {
                        list_day =  Utils.getDaysList(29);
                    } else if (!isRunYear(year) && list_day.size() != 28) {
                        list_day =  Utils.getDaysList(28);
                    }
                    loopView3.setItems(list_day);
        ...
        loopView2.setListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                if (month.equals("02")) {
                    ...
                    list_day =  Utils.getDaysList(31);
                } else if ((month.equals("04") || month.equals("06") || month.equals("09") || month.equals("11"))
                        && list_day.size() != 30) {
                    list_day =  Utils.getDaysList(30);
                }
                loopView3.setItems(list_day);
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (null != onClickOkListener) {
                            ...
                            if (select_type == TYPE_BIRTH &&  Utils.afterToday(date)) {
                                Toast.makeText(context, "选择日期不能超过今天", Toast.LENGTH_SHORT).show();
                            } else if (select_type ==  Utils.TYPE_TODO &&  Utils.beforeToday(date)) {
